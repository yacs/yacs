...
// display the input form
	...
	// tracker
        ...
        if(isset($context['site_trailer']) && $context['site_trailer']) $context['site_tracker'] = $context['site_trailer'];
	
        $label      = i18n::s('Tracking code');
	$input      = '<textarea name="site_tracker" cols="40" rows="2">'.encode_field(isset($context['site_tracker']) ? $context['site_tracker'] : '').'</textarea>';
	$hint       = i18n::s('Additional text inserted after the body openning tag. Use this to integrate page tracking systems, with adequate &lt;script&gt; tags.');
	$fields[]   = array($label, $input, $hint);
        
        $label      = i18n::s('Control tracking cookie');
        $checked    = (isset($context['tracker_cookie_control']) 
              && $context['tracker_cookie_control'] == 'Y'
              && isset($context['cookie_control'])
              && $context['cookie_control'] == 'Y')?'checked':'';
        $disabled   = (!isset($context['cookie_control'])
              || $context['cookie_control'] != 'Y')?'disabled':'';
        $input      = '<input type=checkbox name=tracker_cookie_control value=Y '.$checked.' '.$disabled.'/>';
        $hint       = i18n::s('Check this to submit tracking code to cookie acceptation. Cookie header must be enabled before');
        $fields[]   = array($label, $input, $hint);
	...
	$fields = array();
        ...
        if(!isset($context['cookie_control'])) $context['cookie_control'] = 'N';
        $label      = i18n::s('Display a cookies acceptation header');
        $checked    = ($context['cookie_control'] == 'Y')?'checked':'';
	$input      = '<input type="checkbox" name="cookie_control" value=Y '.$checked.'/>';
	$hint       = i18n::s('Check this to activate a library able to block cookies unless they are allowed by surfer.');
	$fields[]   = array($label, $input, $hint);
        
        if(!isset($context['cookie_message'])) $context['cookie_message'] = ''
              . '[lang=fr]Nous utilisons les cookies, seulement pour le suivi des visites sur notre site, nous ne stockons pas de données personnelles.[/lang]'
              . '[lang=en]We use cookies, just to track visits to our website, we store no personal details.[/lang]';
        $label      = i18n::s('Cookie acceptation header text');
        $input      = '<textarea name=cookie_message cols=80 rows=5 >'.$context['cookie_message'].'</textarea>';
        $hint       = i18n::s('Text displayed by header. You may use Yacs\' Control Code for language version or include links');
        $fields[]   = array($label, $input, $hint);
        
        $cookie .= Skin::build_form($fields);
	$fields = array();
	...
	$all_tabs = array(
		array('meta', i18n::s('Meta-information'), 'meta_panel', $meta),
                array('cookie', i18n::s('Cookies'), 'cookie_panel', $cookie),
...
// save updated parameters
	...
	$content = '<?php'."\n"
		...
		.'$context[\'site_tracker\']=\''.addcslashes($_REQUEST['site_tracker'], "\\'")."';\n";
        
        if(isset($_REQUEST['tracker_cookie_control']))
		$content .= '$context[\'tracker_cookie_control\']=\''.addcslashes($_REQUEST['tracker_cookie_control'], "\\'")."';\n";
        if(isset($_REQUEST['cookie_control']))
		$content .= '$context[\'cookie_control\']=\''.addcslashes($_REQUEST['cookie_control'], "\\'")."';\n";
        if(isset($_REQUEST['cookie_message']))
		$content .= '$context[\'cookie_message\']=\''.addcslashes($_REQUEST['cookie_message'], "\\'")."';\n";
