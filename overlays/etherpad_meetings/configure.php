 ...
 *
 * - [code]etherpad_server[/code] - host name of the server that is running etherpad
 *
 * Configuration information is saved into [code]parameters/overlays.etherpad_meetings.include.php[/code].
 *
...
// save updated parameters
	...
	$content = '<?php'."\n"
		.'// This file has been created by the configuration script overlays/etherpad_meetings/configure.php'."\n"
		.'// on '.gmdate("F j, Y, g:i a").' GMT, for '.Surfer::get_name().'. Please do not modify it manually.'."\n";

	if(isset($_REQUEST['etherpad_server']))
		$content .= '$context[\'etherpad_server\']=\''.addcslashes($_REQUEST['etherpad_server'], "\\'")."';\n";

	$content .= '?>'."\n";

	// update the parameters file
	if(!Safe::file_put_contents('parameters/overlays.etherpad_meetings.include.php', $content)) {

		Logger::error(sprintf(i18n::s('ERROR: Impossible to write to the file %s. The configuration has not been saved.'), 'parameters/overlays.etherpad_meetings.include.php'));

	// report to end-user
	} else {

		$context['text'] .= '<p>'.sprintf(i18n::s('The following configuration has been saved into the file %s.'), 'parameters/overlays.etherpad_meetings.include.php')."</p>\n";
		...
		$menu = array_merge($menu, array( 'overlays/etherpad_meetings/configure.php' => i18n::s('Configure again') ));
