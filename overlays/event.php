...
class Event extends Overlay {
	...
	function get_fields($host, $field_pos=NULL) {
		...
		if(!isset($this->attributes['date_stamp']) || ($this->attributes['date_stamp'] <= NULL_DATE))
			$this->attributes['date_stamp'] = gmdate('Y-m-d H:i', time() + (Surfer::get_gmt_offset() * 3600));
	...
	function remember($action, $host, $reference) {
		...
		case 'delete':
			...
			if(isset($this->attributes['date_stamp']) && ($this->attributes['date_stamp'] > gmdate('Y-m-d H:i'))
