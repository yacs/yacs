	...
	**/
	function &layout(&$result) {
		...
		while($item =& SQL::fetch($result)) {
			...
			$cells[] = '<input type="checkbox" name="sections[]" class="row_selector" value="'.$item['id'].'" />';
...
// 				$details[] = Skin::build_link(Sections::get_url($item['id'], 'rate'), Skin::build_rating_img((int)round($item['rating_sum'] / $item['rating_count'])), 'basic');
		...
		// select all rows
		$cells = array('<input type="checkbox" class="row_selector" onchange="cascade_selection_to_all_section_rows(this);" />', i18n::s('Select all pages'), '');
		$text .= Skin::table_row($cells, $count++);
