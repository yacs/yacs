...
// display the category
    ...
    // inPlaceRichEditor
      ...
      $text .= '  if (!areas[area]) {'."\n";
      $text .= '    areas[area] = new Ajax.InPlaceRichEditor($(area), \''.$context['url_to_root'].'shared/ajax_record_db.php?key=\'+area+\'&current_anchor_class=categories&current_anchor=category:'.$_REQUEST['id'].'\', {
        okText:\'Enregistrer\',cancelText:\'Annuler\',savingText:\'Enregistrement en cours...\',cancelControl:\'button\',onComplete: kill_area(\'+area+\'), loadTextURL: \''.$context['url_to_root'].'shared/ajax_get_value.php?key=\'+area+\'&current_anchor_class=categories&current_anchor=category:'.$_REQUEST['id'].'\'}, tinyMCE.settings);'."\n";
      $text .= '    return false;'."\n";
      $text .= '  }'."\n";
      $text .= '}'."\n";
      $text .= '//]]>'."\n";
      $text .= '</script>'."\n";
		...
		$text .= Skin::build_block($item['description'], 'description', 'description', null, Surfer::is_empowered()?'onclick="edit_area(\'description\');"':'');
