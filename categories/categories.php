...
Class Categories {
	...
	public static function get_by_keyword($keyword) {
		...
		$query = "SELECT * FROM ".SQL::table_name('categories')." AS categories"
			." WHERE categories.keywords RLIKE '\\\b".SQL::escape($keyword)."\\\b'"
                        ." LIMIT 1";
	...
	public static function list_keywords($prefix, $mothercat = NULL) {
		...
		// select among available items
		$query = "SELECT unique keywords FROM ".SQL::table_name('categories')." AS categories"
		...
		if(!isset($context['users_without_archiving']) || ($context['users_without_archiving'] != 'Y')) {
			...
			if(is_string($stamp) && ($stamp > NULL_DATE) && ($stamp = strtotime($stamp)) && ($stamp = getdate($stamp))) {
				...
				if(!($category = Categories::lookup('week '.date('y/m/d', $week))) && ($anchor = Categories::get(i18n::c('weekly')))) {
					...
					$fields['create_date'] = gmdate('Y-m-d H:i:s', $week);
					$fields['edit_date'] = gmdate('Y-m-d H:i:s', $week);
				...
				if(!($category = Categories::lookup('month '.date('M Y', $month))) && ($anchor = Categories::get(i18n::c('monthly')))) {
					...
					$fields['create_date'] = gmdate('Y-m-d H:i:s', $month);
					$fields['edit_date'] = gmdate('Y-m-d H:i:s', $month);
