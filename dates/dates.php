...
Class Dates {
	...
	public static function &list_future_for_anchor($anchor, $offset=0, $count=100, $variant='family', $back_to_first = FALSE) {
		...
		// the request
		$query = "SELECT dates.date_stamp as date_stamp, articles.id as id, articles.title as title, articles.nick_name as nick_name, articles.active, articles.edit_date, articles.publish_date, articles.introduction, articles.thumbnail_url article.anchor FROM ".SQL::table_name('dates')." as dates"
			.", ".SQL::table_name('articles')." AS articles"
			." WHERE ((dates.anchor_type LIKE 'article') AND (dates.anchor_id = articles.id))"
