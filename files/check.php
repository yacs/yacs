...
if(!Surfer::is_associate()) {
	...
	$menu = array('files/' => i18n::s('All Files'));
...
// look for orphans
	...
	// scan files
	$context['text'] .= Skin::build_block(i18n::s('Scanning files table...'), 'subtitle');
	...
	// parse the whole list
		...
		while($row =& SQL::fetch($result)) {
			...
			if($row['anchor'] && !Anchors::get($row['anchor'])) {
				$context['text'] .= sprintf(i18n::s('Orphan: file %s'), Skin::build_link(Files::get_url($row['id']), $row['id'].' '.$row['title'])).BR."\n";
				if(++$errors_count >= 5) {
