...
// look for unused tables
	...
	// scan tables
	$context['text'] .= Skin::build_block(sprintf(i18n::s('Analyzing table %s...'), SQL::table_name('tables')), 'subtitle');
		...
		while($row = SQL::fetch($result)) {
			...
			if(preg_match('/article:(.*)/', $row['anchor'], $matches) && ($article = Articles::get($matches[1]))) {
				...
				if(!preg_match('/\[table='.$row['id'].'.*\]/', $article['description'])) {
					$context['text'] .= sprintf(i18n::s('Unused: %s'), 'table '.Skin::build_link(Tables::get_url($row['id']), $row['id'].' '.$row['title'])).BR."\n";
					if(++$errors_count >= 25) {
	...
	// parse the whole list
		...
		while($row = SQL::fetch($result)) {
			...
			if($row['anchor'] && !Anchors::get($row['anchor'])) {
				$context['text'] .= sprintf(i18n::s('Orphan: %s'), 'table '.Skin::build_link(Tables::get_url($row['id']), $row['id'].' '.$row['title'])).BR."\n";
				if(++$errors_count >= 5) {
						$context['text'] .= i18n::s('Too many successive errors. Aborted').BR."\n";
					break;
				}
