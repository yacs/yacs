<?php
/**
 * move images from users/avatars to skins/images/avatars
 *
...
// splash message
global $local;
$local['label_en'] = 'Move avatars';
$local['label_fr'] = 'D&eacute;placement des avatars';
echo i18n::l($local, 'label')."<br />\n";

// the list of files to move
include_once $context['path_to_root'].'scripts/scripts.php';
$files = Scripts::list_files_at('users/avatars', FALSE);

// the new location for these files
$target = 'skins/images/avatars';
...
foreach($files as $file) {
	...
	// unlink the target, if any
	if(file_exists($context['path_to_root'].$target.'/'.basename($file)))
		Safe::unlink($target.'/'.basename($file));
