...
if(is_object($anchor) && (!$zoom_type || ($zoom_type == 'users'))) {
	...
	if(Surfer::get_id() && !$in_watch_list) {
		$box['bar'] += array(Users::get_url('article:'.$item['id'], 'track') => fa::_("fa-eye").' '.i18n::s('Watch this page'));
	}

	// invite participants, for owners
	if(Articles::is_owned($item, $anchor) && isset($context['with_email']) && ($context['with_email'] == 'Y')) {
		$box['bar'] += array(Articles::get_url($item['id'], 'invite') => fa::_("fa-user-plus").' '.i18n::s('Invite participants'));
	}
	...
	if(($count > 1) && Articles::allow_message($item, $anchor) && isset($context['with_email']) && ($context['with_email'] == 'Y')) {
		...
		$box['bar'] += array(Articles::get_url($item['id'], 'mail') => fa::_("fa-envelope-o").' '.i18n::s('Notify participants'));
	}

	// manage editors, for owners
	if(Articles::is_owned($item, $anchor, TRUE) || Surfer::is_associate()) {
		$box['bar'] += array(Users::get_url('article:'.$item['id'], 'select') => fa::_("fa-share-alt").' '.i18n::s('Manage participants'));

	// leave this page, for editors
	} elseif(Articles::is_assigned($item['id'])) {
		$box['bar'] += array(Users::get_url('article:'.$item['id'], 'leave') => fa::_("fa-share-alt").' '.i18n::s('Leave this page'));
	}
