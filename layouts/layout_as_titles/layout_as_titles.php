	...
	function layout($result) {
		...
		if(!SQL::count($result))
			return $text;
                ...
                // flex variant
                $flex_layout = $this->has_variant('flex');
		...
		while($item = SQL::fetch($result)) {
			...
			elseif($item['edit_date'] >= $context['fresh'])
				$suffix .= UPDATED_FLAG;
                        ...
                        if(!$this->has_variant('no_details')) {
			...
			// add a floating or header box
                        $box_variant = ($flex_layout)?'header2':'floating';
			$text       .= Skin::build_box(NULL, $content, $box_variant);
                                          

		}
                
                if($flex_layout) {
                    $text = tag::_('div',tag::_class('k/flex-container'),$text);
                } else {
