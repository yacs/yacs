...
var TreeManager = {
	...
	elems.click(function(e){
		e.stopPropagation();
		// find parent folder
		var li = $(this).closest('.tm-drop');
		var top_li = $(this).closest('.tm-masonry');
		...
		subul.toggle(100,function() {
		    ...
		    TreeManager.checkHeight(top_li,'lay');
		});
	    });
    },
	...
	if(list.hasClass('tm-root') && !obj.hasClass('tm-masonry') && objIsFolder) {
	    ...
	    // remove special attributes of folders on root
	...
	obj.animate({left:'0',top:'0'},100,
		...
		// bug : remove tm-hoverdrop class 
		// that sometimes still stick to dropable
		$('.tm-ddz, .tm-drop').removeClass('tm-hoverdrop');
	    }
	);
    },
	...
	var ddzVariant = anchor.closest('.tm-ddz').data('variant');	

	Yacs.startWorking();
	$.post(
	    tm_ajaxUrl,
	    {action : 'create', anchor : anchor.data('ref'), title : input.val(), variant:ddzVariant}
	).done(function( data ) {
	...
	$.get(
	    tm_ajaxUrl,
	    {action : 'zoom', anchor : anchor.data('ref'), variant:ddzVariant}
	).done(function( data ) {
